...
public abstract class AtomHBaseHelper {
	...
	public static final byte[] Q_NB_PRICES_FIXED = Bytes
			.toBytes("NbPricesFixed");
	public static final byte[] Q_LAST_FIXED_PRICE = Bytes
			.toBytes("LastFixedPrice");
	...
	public static final byte[] Q_FIRST_FIXED_PRICE = Bytes
			.toBytes("FirstFixedPrice");
	...
	public void setTableName(TableName tableName) {
		if (this.open)
			throw new HadoopTutorialException("TableName already set to "
					+ this.tableName);
		this.tableName = tableName;
	}
	...
	public void setColumnFamily(byte[] columnFamily) {
		if (this.open)
			throw new HadoopTutorialException("ColumnFamily already set to "
					+ this.columnFamily);
		this.columnFamily = columnFamily;
	}
	...
	public void setHbaseConfiguration(Configuration hbaseConfiguration) {
		if (this.open)
			throw new HadoopTutorialException("hbaseConfiguration already set");
	...
	public void closeTable() {
		...
		try {
			...
			throw new HadoopTutorialException("Failed to push data into queue",
					e);
		}
	...
	protected Table getTable() throws HadoopTutorialException {
		try {
			LOGGER.log(Level.FINEST,
					"Getting table " + tableName.getQualifierAsString());
			if (useBuffer) {
				if (!open) {
					openTable();
				}
	...
	protected void returnTable(Table table) throws HadoopTutorialException {
		try {
			LOGGER.log(Level.FINEST, "Returning table "
					+ table.getName().getQualifierAsString());
			if (!useBuffer) {
				table.close();
			}
		} catch (IOException e) {
	...
	private void createTable(Connection connection) {
								...
								// de table
		{
			...
			throw new HadoopTutorialException(
					"Cannot create table (should be a zombie)", e);
		}
		// @TODO should be manage in a thread safe manner?
		open = true;
	}
	...
	final public void openTable() {
		 ...
		 * (IOException e) { // can't do anything here... } }
		 */
	}
	...
	public static TraceType lookupType(@NotNull byte[] row) {
		...
		switch (c) {
		case 'O':
			type = TraceType.Order;
			break;
		case 'P':
			type = TraceType.Price;
			break;
		case 'E':
			type = TraceType.Exec;
			break;
	...
	public Put mkPutAgent(byte[] row, long ts, Agent a, Order o, PriceRecord pr) {
		...
		p.addColumn(columnFamily, Q_PRICE, ts, hbEncoder.encodeLong(pr.price));
	...
	@NotNull
	protected Put mkPutAgentReferential(@NotNull AgentReferentialLine agent,
			@NotNull HBaseDataTypeEncoder encoder, @NotNull byte[] columnF,
			long ts) {
		Put p = new Put(Bytes.toBytes(agent.agentRefId + "R"), ts);
		p.addColumn(columnF, Bytes.toBytes("agentRefId"), ts,
				encoder.encodeInt(agent.agentRefId));
		p.addColumn(columnF, Bytes.toBytes("agentName"), ts,
				encoder.encodeString(agent.agentName));
		p.addColumn(columnF, Bytes.toBytes("isMarketMaker"), ts,
				encoder.encodeBoolean(agent.isMarketMaker));
		p.addColumn(columnF, Bytes.toBytes("details"), ts,
				encoder.encodeString(agent.details));
		p.addColumn(columnFamily, Q_TIMESTAMP, ts, hbEncoder.encodeLong(ts));
		return p;
	}

	protected Put mkPutDay(byte[] row, long ts, int dayGap, int nbDays,
			OrderBook ob) {
		Put p = new Put(row, ts);
		p.addColumn(columnFamily, Q_TRACE_TYPE, ts,
	...
	protected Put mkPutPriceRecord(byte[] row, long ts, PriceRecord pr,
			long bestAskPrice, long bestBidPrice) {
		Put p = new Put(row, ts);
		p.addColumn(columnFamily, Q_TRACE_TYPE, ts,
		...
		p.addColumn(columnFamily, Q_EXECUTED_QUANTITY, ts,
				hbEncoder.encodeInt(pr.quantity));
		p.addColumn(columnFamily, Q_DIR, ts, hbEncoder.encodeChar(pr.dir));
	...
	protected Put mkPutTick(byte[] row, long ts, int dayGap, Day day,
			OrderBook ob) {
		Put p = new Put(row, ts);
		p.addColumn(columnFamily, Q_TRACE_TYPE, ts,
	...
	public void setUseBuffer(boolean useBuffer) {
		if (this.open)
			throw new HadoopTutorialException("TableName already open "
					+ this.tableName);
